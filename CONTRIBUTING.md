...
Playwright e2e is **local-only** — not a GitHub Actions gate. CI runs **quality** only (`pnpm lint`, `pnpm test:types`, `pnpm test:run`). Locally, run the same gate with `pnpm ci:quality` before a release.
